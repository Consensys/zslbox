...
// func parseAuthPath(authPath []string) ([29][32]byte, error) {
// 	var toReturn [29][32]byte

// 	if len(authPath) != 29 {
// 		return toReturn, fmt.Errorf("authentication path must be %d in length", 29)
// 	}

// 	for i := 0; i < 29; i++ {
// 		b, err := hex.DecodeString(strings.TrimPrefix(authPath[i], "0x"))
// 		if err != nil {
// 			return toReturn, fmt.Errorf("error while decoding authpath[%d] - %s. is it hex encoded ?", i, authPath[i])
// 		}
// 		var uncle [32]byte
// 		copy(uncle[:], b[:32])
// 		toReturn[i] = uncle
